    ...
    <body class="min-h-screen bg-slate-50">
        <main class="mx-auto flex min-h-screen max-w-md items-center px-6">
            <div class="w-full rounded-2xl bg-white p-8 shadow-lg">
                <div class="mb-6">
                    <h1 class="text-2xl font-semibold text-slate-900">Activate your account</h1>
                    <p class="mt-2 text-sm text-slate-600">
                        Complete your invitation as {{ str($invitation->role)->replace('_', ' ')->title() }}.
                    </p>
                </div>
                ...
                @else
                    <form method="POST" action="{{ route('user.invitation.accept', $invitation->token) }}" class="space-y-4">
                        @csrf
